...
exports.getParamName =
  exports.renderParamSignature =
  exports.renderOperationGroup =
  exports.genOperationGroupFiles =
    void 0;
...
function genOperationGroupFiles(spec, operations, options) {
  ...
  for (let name in groups) {
    ...
    (0, util_1.join)(
      lines,
      renderOperationGroup(group, renderOperation, spec, options)
    );
...
function renderDocParam(param) {
  ...
  return `${support_1.DOC}@param {${(0, support_1.getDocType)(
    param
  )}} ${name} ${description}`;
}
function renderDocReturn(op) {
  ...
  description = description
    .trim()
    .replace(/\n/g, `\n${support_1.DOC}${support_1.SP}`);
  return `${support_1.DOC}@return {Promise<${(0, support_1.getDocType)(
    response
  )}>} ${description}`;
}
...
function getParamName(name) {
  ...
  const reduced = parts.reduce(
    (name, p) => `${name}${p[0].toUpperCase()}${p.slice(1)}`
  );
...
function renderOperationObject(spec, op, options) {
  ...
  names.forEach((name, i) => {
    (0, util_1.join)(
      lines,
      renderParamGroup(name, parameters[name], i === last)
    );
  });
  if (lines.length) {
    if (options.language === "ts") {
      lines.unshift(
        `${support_1.SP}const parameters: api.OperationParamGroups = {`
      );
    } else {
...
function groupParams(groups, param) {
  ...
  if (param.type === "array") {
    if (!param.collectionFormat)
      throw new Error(
        `param ${param.name} must specify an array collectionFormat`
      );
...
function renderRequestCall(op, options) {
  ...
  return [
    `${support_1.SP}return gateway.request(${op.id}Operation${params})${support_1.ST}`,
    "}",
  ];
}
function renderOperationParamType(spec, op, options) {
  ...
  const lines = [];
  lines.push(
    `export interface ${op.id[0].toUpperCase() + op.id.slice(1)}Options {`
  );
  optional.forEach((param) => {
    ...
    lines.push(
      `${support_1.SP}${getParamName(param.name)}?: ${(0,
      support_1.getTSParamType)(param)}${support_1.ST}`
    );
  });
  lines.push("}");
  lines.push("");
...
function renderOperationInfo(spec, op, options) {
  ...
  if (hasBody && op.contentTypes.length) {
    lines.push(
      `${support_1.SP}contentTypes: ['${op.contentTypes.join("','")}'],`
    );
  }
...
function renderSecurityInfo(security) {
  return security
    .map((sec, i) => {
      ...
      secLines.push(
        `${support_1.SP.repeat(3)}id: '${sec.id}'${scopes ? "," : ""}`
      );
      if (scopes) {
        secLines.push(
          `${support_1.SP.repeat(3)}scopes: ['${scopes.join(`', '`)}']`
        );
      }
      secLines.push(
        `${support_1.SP.repeat(2)}}${i + 1 < security.length ? "," : ""}`
      );
      return secLines;
    })
    .reduce((a, b) => a.concat(b));
