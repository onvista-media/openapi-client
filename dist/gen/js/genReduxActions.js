...
function genReduxActionGroupFiles(spec, operations, options) {
  ...
  for (let name in groups) {
    ...
    lines.push(
      (0, genOperations_1.renderOperationGroup)(
        group,
        renderReduxActionBlock,
        spec,
        options
      )
    );
...
function renderReduxActionBlock(spec, op, options) {
  ...
  let paramSignature = (0, genOperations_1.renderParamSignature)(
    op,
    options,
    `${op.group}.`
  );
  ...
  let params = required
    .map((param) => (0, genOperations_1.getParamName)(param.name))
    .join(", ");
...
export const ${actionComplete} = 's/${op.group}/${actionComplete}'${
    support_1.ST
  }
